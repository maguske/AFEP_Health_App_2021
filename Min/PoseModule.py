...
class poseDetector:
    ...
    def angle_between_two_points(self, img, point_a=0, point_b=1, draw=True):
        if self.results.pose_landmarks:
            ...
            denominator = math.sqrt(lma.x ** 2 + lma.y ** 2 + lma.z ** 2) * math.sqrt(
                lmb.x ** 2 + lmb.y ** 2 + lmb.z ** 2)
...
def main():
    ...
    while True:
        ...
        if len(pose_landmarks) != 0:
            cv2.circle(img, (pose_landmarks[14][1], pose_landmarks[14][2]), 7, (0, 255, 0), cv2.FILLED)
